...
class Simulation {
    ...
    start() {
        ...
        const multiMatrix = {w: 8, h: 12}, shiftsHor = [], shiftsVert = [];

        const w = 940, margin = 3, vertMargin = 3;

        //const gMargin = 42;
        const gMargin = 32;

        const shiftAdd = ((w - (2 * margin) - ((multiMatrix.w - 1) * gMargin)) / multiMatrix.w);
        ...
        for (let i = 0; i < shiftsVert.length; i++) {
            for (let j = 0; j < shiftsHor.length; j++) {
                graphics.push({
                    g: svg.append("g").attr("transform",
                        "translate(" + shiftsHor[j] + ", " + shiftsVert[i] + ")")
                });
            }
        }
        ...
        let simulate = function () {
            for (let i = 0; i < graphics.length; i++) {
                let element = graphics[i];
                let game = new Game(element.g, params, 0, styles);
                game.play();
            }
            // for (let element in graphics) {
            //     let game = new Game(element.g, params, 0, styles);
            //     game.play();
            // }
...
class Piece {
    ...
    constructor(board, startingPoint, white) {
        ...
        this.lineColor = clr.white;
        this.lineWeight = wghts.normal;
    ...
    static getStyleDefault() {
        return {r: 9, pieceStrokeWidth: 3, lineStrokeWidth: 5.8};
    }

    static getStyleFat() {
        return {r: 9, pieceStrokeWidth: 4, lineStrokeWidth: 3.8};
    }
...
class Queen extends Piece {
    ...
    constructor(board, startingPoint, white, delay = 0, advance = false, style = null) {
        super(board, startingPoint, white);
        this.lineColor = clr.gold;
        this.lineWeight = wghts.ultra;
        this.genericMatrix = [];
...
class Knight extends Piece {
    ...
    constructor(board, startingPoint, white) {
        super(board, startingPoint, white);
        this.lineColor = clr.green;
        this.lineWeight = wghts.fat;
        this.genericMatrix = [
    ...
    play() {
        ...
        this.generateLines().forEach(function (line, index) {
            ...
            pth.attr("stroke-dasharray", totalLength + " " + totalLength)
                ...
                .attr("stroke-dashoffset", 0);
            // .transition()
            // .delay(Board.randomInt(lout.delayMin, lout.delayMax) + eraseDelay)
            // .duration(Board.randomInt(lout.durMin, lout.durMax))
            // .ease(d3.easePolyOut)
